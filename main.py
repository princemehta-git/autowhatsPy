...
def open_driver(profile:str, link: str = 'https://web.whatsapp.com', headless: str = False,
...
def profile_logout(profile: str):
    # open_driver(profile)
    # print('driver opened ')
    check = open_driver(profile,check_login=True)
    if check[0]:
...
def send(profile:str = None, excel_path: str = None, gap_set:int = 1,
    ...
    if open_driver(profile=profile, check_login=True)[0]:
        ...
        for track_row in range(2,total_row+1):
            ...
            if num:
                driver.get('https://web.whatsapp.com/send?phone=' + num)
                send_box = locate_element(xpath='//div[@title="Type a message"]', wait_time=20)
